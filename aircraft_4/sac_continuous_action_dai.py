...
def parse_args():
    ...
    parser.add_argument("--wandb-project-name", type=str, default="aircraft_exp1",
        help="the wandb's project name")
    parser.add_argument("--wandb-entity", type=str, default="collaboratewithme",
        help="the entity (team) of wandb's project")
    parser.add_argument("--capture-video", type=lambda x: bool(strtobool(x)), default=False, nargs="?", const=True,
        help="whether to capture videos of the agent performances (check out `videos` folder)")

    # Algorithm specific arguments
    parser.add_argument("--env-id", type=str, default="aircraft-v0",
...
if __name__ == "__main__":
    args = parse_args()
    run_name = f"{args.exp_name}__{args.seed}__obstacleNum{args.number_of_obstacle}__{int(time.time())}"
    if args.track:
        import wandb
    ...
    # TRY NOT TO MODIFY: start the game
    obs = envs.reset()
    while (global_episode < args.total_episodes):
        global_step += 1
        ...
        for info in infos:
            if "episode" in info.keys():
                ...
                writer.add_scalar("to_be_ploted/episodic_return", info["episode"]["r"], global_episode)
                writer.add_scalar("charts/episodic_length", info["episode"]["l"], global_episode)
                break
            ...
            if global_step % args.policy_frequency == 0:  # TD 3 Delayed update support
                for _ in range(
                    ...
                    if args.autotune:
                        ...
                        alpha = alpha*args.alpha_coefficient
